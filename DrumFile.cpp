...
const TCHAR * szFilter[] = { TEXT("Drum Files (*.DRM)"), TEXT("*.drm"), TEXT("") };
...
TCHAR * DrumFileWrite(DRUM * pdrum, TCHAR * szFileName)
{
	...
	if ((hmmio = mmioOpen(szFileName, NULL, MMIO_CREATE | MMIO_WRITE | MMIO_ALLOCBUF)) == NULL)
	...
	wError |= (mmioWrite(hmmio, szSoftware, sizeof(szSoftware)) != sizeof(szSoftware));
	...
	wError |= mmioCreateChunk(hmmio, &mmckinfo[2], 0);
	wError |= (mmioWrite(hmmio, szDateBuf, (strlen(szDateBuf) + 1)) != (int)(strlen(szDateBuf) + 1));
	wError |= mmioAscend(hmmio, &mmckinfo[2], 0);
	...
	wError |= (mmioWrite(hmmio, (PSTR)&iFormat, sizeof(int)) != sizeof(int));
	wError |= mmioAscend(hmmio, &mmckinfo[1], 0);
	// Create the "data" sub-chunk
	mmckinfo[1].ckid = mmioStringToFOURCC(szDataID, 0);
	wError |= mmioCreateChunk(hmmio, &mmckinfo[1], 0);
	wError |= (mmioWrite(hmmio, (PSTR)pdrum, sizeof(DRUM)) != sizeof(DRUM));
	wError |= mmioAscend(hmmio, &mmckinfo[1], 0);
